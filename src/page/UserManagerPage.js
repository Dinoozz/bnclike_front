...
const UserManager = () => {
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} className="rounded-full bg-green-500 p-4 m-4 z-10">
      ...
      {showModal && (
        <Modal onClose={() => setShowModal(false)}>
          <form onSubmit={(e) => { e.preventDefault(); createUser(); }} className="space-y-4">
            <input className="border-2 pl-1 border-white-100" type="text" name="username" value={newUser.username} onChange={(e) => handleInputChange(e, newUser, setNewUser)} placeholder="Nom d'utilisateur"/>
            <input className="border-2 pl-1 border-white-100" type="email" name="email" value={newUser.email} onChange={(e) => handleInputChange(e, newUser, setNewUser)} placeholder="Email"/>
            <input className="border-2 pl-1 border-white-100" type="password" name="password" value={newUser.password} onChange={(e) => handleInputChange(e, newUser, setNewUser)} placeholder="Password"/>
